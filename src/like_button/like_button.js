...
export function like() {
    // let flag = {Value}
    let like = document.getElementsByClassName("white")
    let border = document.getElementsByClassName("like_button")
    console.log("dd")
    for(let i = 0; i < like.length; i++){
        border[i].addEventListener("click", () => liked(like, i, border), false)
        console.log("dd")
    }
    let flag = {Value: new Array(like.length)}
    for(let i = 0; i < like.length; i++) {
        flag.Value[i] = true
    ...
    function liked(like, i) {
        ...
        if(flag.Value[i]) {
            ...
            $(like[i]).children('p').css({"color":"#BC9CFF"})
            $(like[i]).children('p').html(parseInt($(like[i]).children('p').html()) + 1)

            $(border[i]).css({"background":"linear-gradient(180deg, #BC9CFF 0%, #8BA4F9 100%)"})
            ...
            $(like[i]).children('p').css({"color":"rgba(31, 32, 65, 0.25)"})
            $(like[i]).children('p').html(parseInt($(like[i]).children('p').html()) - 1)


            $(border[i]).css({"background":"rgba(31, 32, 65, 0.25)"})
